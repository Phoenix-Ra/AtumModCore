...
public class GuiOptionsExtended extends GuiOptions {
    ...
    public GuiOptionsExtended(GuiScreen guiScreen, GameSettings gameSettings) {
        ...
        windowSizeType = DisplayResolution.getCurrentGlobalResolutionIndex();
    }
    ...
    public void initGui() {
        ...
        this.buttonList.add(new WindowSizeSlider(
                        ...
                        this.height / 6 + 144 - 6,
                        0,
                        DisplayResolution.getSupportedResolutions().size() - 1
                )
        );
    }
    ...
    public void onGuiClosed() {
        super.onGuiClosed();
        try {
            DisplayResolution.setResolution(windowSizeType);
        }catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    private class WindowSizeSlider extends GuiButton {
        ...
        public WindowSizeSlider(int buttonId, int x, int y, float minValueIn, float maxValue) {
            ...
            this.displayString = "Window size: " + Display.getWidth()
                    + "x" + Display.getHeight()
        ...
        protected void mouseDragged(Minecraft mc, int mouseX, int mouseY) {
            if (this.visible) {
                if (this.dragging) {
                    ...
                    PairRecord<DisplayResolution, PairRecord<Integer,Integer>>
                            resolution = DisplayResolution.getSupportedResolutions()
                            .get(windowSizeType);

                    this.displayString = "Window size: " + resolution.getSecond().getFirst()
                            + "x" + resolution.getSecond().getSecond()
                            + " (" + (resolution.getFirst().name().substring(1)) + ")";;
                }
